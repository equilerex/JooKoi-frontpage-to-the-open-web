...
- **Two documented exceptions to "everything under `:host`".** `hardware-key` styles an inner element, because its slab is a child `.key` rather than the host itself; `prose-content` needs `:host x` descendant rules for the long-form HTML a consumer projects into it. Anything else that has to reach beyond its own box is a library-internal case and belongs in the global layer (ADR 014), not here.
