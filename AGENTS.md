...
- **Internal navigation must use Angular Router tooling (`RouterLink` / `Router.navigate`), never raw `href`.** Raw `<a href="/...">` triggers a full browser page load, wipes application and signal state, and breaks deployments on subpaths (like GitHub Pages) where root-relative URLs bypass `<base href>`. Raw `href` is reserved strictly for external outbound links (with `target="_blank"`).
