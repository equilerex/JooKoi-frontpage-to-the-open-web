...
- [x] **Replace the failed motion prototype** — audit and roadmap in `plans/2026-09-22-motion.md`; route, filter-state, library-reader, and interaction recipes implemented. Search filters use writable signals and `Location.replaceState` to prevent router navigation and scroll-to-top (ADR 035). Record grid rows animate enter/leave via safe GPU compositor opacity transitions with stable `rowKey` tracking and 120ms keyword input debouncing (ADR 036).
...
- [x] **Performance pass and Lighthouse baselines** — decisions 033, 037 (restored PrimeNG Table with lazy route chunks and virtual scrolling, superseding 032). Home lazy, shell status count deferred, budgets re-set, `learn/:topic` `RenderMode.Client`, `lighthouse` devDependency, `ux:lab` record/compare/strict, `ci.yml`. Briefing for the skill that guided it: `jookoi-ai-market/_architecture/plans/angular-skill-improvement-brief.md`.
