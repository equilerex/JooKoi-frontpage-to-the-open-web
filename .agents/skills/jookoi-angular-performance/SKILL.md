...
description: Angular performance advisor and auditor. Find and fix slow spots in an existing Angular app, and answer the deeper "how do I make this faster / how would I approach this" questions - bundle and lazy-chunk size, lazy routes, preloading, @defer, images, fonts, third-party scripts, data loading, SSR and hydration, change detection (OnPush, signals, zoneless), INP/RxJS/forms/list cost, build and deploy settings, and setting up repeatable measurement (Lighthouse, Chrome DevTools MCP, baselines, before/after comparison). Use this whenever the user says an Angular app is slow, janky, heavy, or failing a budget or Lighthouse score, asks to optimize, audit, speed up, shrink the bundle, fix LCP/INP/CLS, go zoneless, plan a performance project, track performance over time, or asks how something performance-related works. Performance only - for writing new Angular features or general best practices use angular-developer, for scaffolding use angular-new-app.
---
